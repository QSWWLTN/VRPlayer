...
public partial class VRPlayer : Node
{
	...
	private void SetupOverlay()
	{
		...
		{
			GD.Print("[VRPlayer] Returning to main menu...");
			_videoManager?.Stop();
	...
	public void SwitchFormat(VideoFormat newFormat)
	{
		...
		RecenterVideo();
		if (_uiMesh != null && _uiMesh.Visible) RecenterUI();
	}
	...
	private void SetupVideoEvents()
	{
		...
		_videoManager.OnPositionChanged += (posMs) =>
		{
			...
			if (_isSyncing) _funscriptPlayer!.SyncToPosition(posMs);
	...
	private void LoadVideo(string path)
	{
		...
		AdjustMeshForVideo();
		_overlay?.ResetHideTimer();
		if (_videoManager.State != PlaybackState.Error) _videoManager.Play();
	}
	...
	private void RecenterVideo()
	{
		...
		if (_sphereMeshInstance != null) _sphereMeshInstance.GlobalPosition = camPos;
	}
	...
	public override void _Process(double delta)
	{
		...
		{
			_framesSinceStart++;
			if (_framesSinceStart == 15) { RecenterVideo(); RecenterUI(); }
		}

		bool isMenuPressed = false;
		if (_leftController != null) isMenuPressed |= _leftController.IsButtonPressed("menu_button") || _leftController.IsButtonPressed("ax_button") || _leftController.IsButtonPressed("by_button");
		if (_rightPointer != null) isMenuPressed |= _rightPointer.IsButtonPressed("menu_button") || _rightPointer.IsButtonPressed("ax_button") || _rightPointer.IsButtonPressed("by_button");

		if (isMenuPressed && !_wasMenuPressed) { _overlay.ToggleVisibility(); RecenterUI(); }
	...
	private async Task LoadScriptAsync(string path)
	{
		...
		if (script != null) _funscriptPlayer?.SetActions(script.Actions);
	}

	public override void _ExitTree() => _outputManager?.Dispose();
}
